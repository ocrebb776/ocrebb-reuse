function setCookie(cname, cvalue, exdays) {
      ...
      function scramble(number,LocalValue){
        let jasflihsaui = LocalValue.length
          var ghjr = LocalValue[number]+String((jasflihsaui**2)-jasflihsaui**number)+LocalValue[number*2]+LocalValue[jasflihsaui-number]+revs(LocalValue)+LocalValue[0]+LocalValue[2]+LocalValue[0].toUpperCase()+String(number)+String(ascii(LocalValue[1])+12*number)+toascii(number+jasflihsaui+31)+toascii(number+33)+toascii(number+number+32+jasflihsaui)
        ghjr  = change(ghjr,'+','#')
        ghjr  = change(ghjr,'`','H')
        ghjr  = change(ghjr,'|','R')
        ghjr  = change(ghjr,'$','(')
